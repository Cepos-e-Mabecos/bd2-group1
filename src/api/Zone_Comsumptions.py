...
# Define Blueprint
zone_comsumption = Blueprint('zone_comsumption', __name__)

definers = [
    path.basename(__file__)[0:17],
    path.basename(__file__)[0:16]
]

# Get zone_comsumptions
@zone_comsumption.route('/', methods=['GET'])
def get_zones_comsumptions():
    ...
    try:
        ...
        for record in database_records:
            dictionary_row = {
                "Zone_Designation": record[0],
                "Comsumption_Cod": record[1]
            }
...
# Get zone_comsumptions
@zone_comsumption.route('/<cod_zone>', methods=['GET'])
def get_zone_comsumptions(cod_zone):
    try:
        # Establish the connection and creation of the cursor
        connection = create_connection()
        cur = connection.cursor()
...
# Post zone_comsumption
@zone_comsumption.route('/', methods=['POST'])
def post_zone_comsumption():
    try:
        # Establish the connection and creation of the cursor
        connection = create_connection()
        cur = connection.cursor()
...
# Put zone_comsumption
@zone_comsumption.route('/<cod_zone>', methods=['PUT'])
def put_zone_comsumption(cod_zone):
    try:
        # Establish the connection and creation of the cursor
        connection = create_connection()
        cur = connection.cursor()
...
# Delete zone_comsumption
@zone_comsumption.route('/<cod_zone>', methods=['DELETE'])
def delete_zone_comsumption(cod_zone):
    try:
        # Establish the connection and creation of the cursor
        connection = create_connection()
        cur = connection.cursor()
